...
public class PartyServiceImpl implements PartyService {
  ...
  private final BiFunction<String, PartyTO.IdentifyingCode, PartyIdentifyingCode> mapIdcCodeToPartyIdc = (partyId, idc) -> {
    ...
    if (dcsaCode == null) {
      if (idc.getCodeListResponsibleAgencyCode() == null) {
        throw ConcreteRequestErrorMessageException.invalidInput("Either DCSAResponsibleAgencyCode or codeListResponsibleAgencyCode must be provided", null);
      }
      try {
        dcsaCode = DCSAResponsibleAgencyCode.legacyCode2DCSACode(idc.getCodeListResponsibleAgencyCode());
      } catch (IllegalArgumentException e) {
        throw ConcreteRequestErrorMessageException.invalidInput("Unknown codeListResponsibleAgencyCode!", null);
      }
      ...
      if (!dcsaCode.getLegacyAgencyCode().equals(idc.getCodeListResponsibleAgencyCode())) {
        throw ConcreteRequestErrorMessageException.invalidInput("DCSAResponsibleAgencyCode and codeListResponsibleAgencyCode do not match. "
          + dcsaCode + " (" + dcsaCode.getLegacyAgencyCode() + ") vs. " + idc.getCodeListResponsibleAgencyCode(), null);
      }
    }
  ...
  public Mono<PartyTO> findTOById(String partyID) {
    return partyRepository.findById(partyID)
      .switchIfEmpty(Mono.error(ConcreteRequestErrorMessageException.notFound("Cannot find party with ID: " + partyID)))
